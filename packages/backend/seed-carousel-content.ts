...
const BRAND_ID = "09dc8dec-34e9-4588-97d9-031b820c8a64"; // Mais.car
...
const carousels: Carousel[] = [
  {
    name: "Autoridade técnica (checklist)",
    backgroundPrompt:
  ...
  {
    name: "Checklist grátis (lead magnet)",
    backgroundPrompt:
  ...
  {
    name: "Diferenciação e confiança",
    backgroundPrompt:
  ...
  {
    name: "Notícia: fraude de quilometragem",
    backgroundPrompt: "close-up of a car odometer dashboard display showing kilometers, dramatic lighting, photorealistic",
    slides: [
  ...
  {
    name: "Notícia: recall Volkswagen",
    backgroundPrompt: "row of cars in a service center for recall repair, mechanics working, photorealistic",
    slides: [
    ...
    for (let i = 0; i < carousel.slides.length; i++) {
      ...
      const content = await prisma.content.create({
        data: {
          ...
          type: "PHOTO",
          status: "REVIEW",
          title: `${carousel.name} — slide ${i + 1}/${carousel.slides.length}`,
          hook: slideText,
        },
      });
      ...
      await prisma.contentAsset.create({
        data: {
          ...
          type: "image",
          url: `${API_BASE}${composed.url}`,
          provider: "pollinations+composer",
        },
      });

      console.log(`  slide ${i + 1}/${carousel.slides.length} -> content ${content.id}`);
    }
  }
  console.log("\nDone.");
}
